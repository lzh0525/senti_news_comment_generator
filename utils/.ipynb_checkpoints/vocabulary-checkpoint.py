...
class Vocab(object):
    ...
    def __init__(self, vocab_list, max_size):
        ...
        for vocab in vocab_list:
            ...
            if w in self._word_to_id:
                print('Duplicated word in vocabulary file Line:', (cnt, w))
                continue
            self._word_to_id[w] = self._count
            self._id_to_word[self._count] = w
            self._count += 1
    ...
    def word_list(self):
        ...
        return list(self._word_to_id.keys())
if __name__=="__main__":
    vocab_list=[]
    for line in open("../data/vocab_entertainment.txt", "r"):  # 设置文件对象并读取每一行文件
        vocab_list.append(line[:-1])
    print("[INFO] vocab_list读取成功！")
    print("[INFO] vocab_size:" , len(vocab_list))
    # 创建vocab类
    vocab = Vocab(vocab_list, 100000)
    print(vocab.id2word(1))
    print(vocab.word2id('发布'))
    print(vocab.word2id('演戏'))
    print(vocab.word2id('知道'))